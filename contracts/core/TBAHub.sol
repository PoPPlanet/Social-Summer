 ...
 */
contract TBAHub is VersionedInitializable, PoPPMultiState, PoPPHubStorage, IPoPPHub {
...
//        require(IERC20(USDT_ADDRESS).transferFrom(msg.sender, summerAwardNightAddress, price*NIGHT/BASE), 'Pay fail.');
    ...
    function proxyCreateAccount(DataTypes.CreateProfileData calldata vars, uint256 price)
    external
    whenNotPaused payable
    returns (uint256)
    {
...
//        require(IERC20(USDT_ADDRESS).transferFrom(msg.sender, summerAwardNightAddress, price*NIGHT/BASE), 'Pay fail.');
    ...
    function luck(uint256 level, uint256 round, uint256 times, uint256 start, uint256 end) external {
        ...
        uint256 luckNumber = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, blockhash(block.number - 1)))) % MAX_RANGE + start;
    ...
    function luckBlack(uint256 round, uint256 start, uint256 end) external {
        ...
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, block.difficulty, blockhash(block.number - 1)))) % 100;
        if (randomNumber < 10) {
            uint256 MAX_RANGE = end - start + 1;
            uint256 luckNumber = uint256(keccak256(abi.encodePacked(msg.sender, block.timestamp, blockhash(block.number - 1)))) % MAX_RANGE + start;
